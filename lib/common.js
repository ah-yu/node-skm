 ...
 * @Last Modified time: 2017-11-17 18:06:25
 */
'use strict'
...
class Common {
    ...
    deleteKey(alias) {
        ...
        this.unlink(path.join(skmPath, alias), (error) => {
            if (error) throw error;
            console.log('delete SSH key failed!');
        })
    ...
    createKey(alias, comment) {
        ...
        if (sshKeyMap.get(alias)) {
            console.log(`${alias} ssh key is already exsit`);
            return;
        }
